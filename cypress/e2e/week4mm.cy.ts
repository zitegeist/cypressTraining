describe('Logged in | Basket Tests', () => {
  ...
  it('Logged in | Add multiple items to basket and remove', () => {
    ...
    cy.get('.cart_price').should('contain.text', 'Rs. 400')
    cy.get('.cart_price').should('contain.text', 'Rs. 700')
    ...
    cy.get('.cart_price').should('not.contain.text', 'Rs. 400')
    cy.get('.cart_price').should('contain.text', 'Rs. 700')
  })
})
describe('Logged out | Basket Tests', () => {
  ...
  it('Logged out | Add multiple items to basket and remove', () => {
    cy.visit('/')
    cy.get('[data-product-id="2"]').first().click()
    cy.get('#cartModal').should('be.visible')
    cy.contains('Added!').should('be.visible')
